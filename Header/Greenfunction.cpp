    ...
    switch (i) {
        ...
        case 'r':
            return -1.0 / 12.0 * (tau - tm) * pow(tau - tp, 2) * (3 * pow(tau, 2) + 2 * tau * tp + pow(tp, 2)) /
                   (mpl * (tau - tp) - mpr * (tau - tm));
        default:
            printError("AGM::Greenfunction::integrate_square", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_linear(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_linear", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_const(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_const", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
    ...
    switch (i) {
        ...
        case 'r':
            return -1.0 / 3.0 * (tau - tm) * (tau - tp) * (pow(tau, 2) + tau * tp + pow(tp, 2)) /
                   (mpl * (tau - tp) - mpr * (tau - tm));
        default:
            printError("AGM::Greenfunction::integrate_square_t", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_linear_t(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_linear_t", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_const_t(char i) const -> double {
    switch (i) {
        ...
        case 'r':
            return -(tau - tm) * (tau - tp) / (mpl * (tau - tp) - mpr * (tau - tm));
        default:
            printError("AGM::Greenfunction::integrate_const_t", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
    ...
    switch (i) {
        ...
        case 'r':
            return -1.0 / 12.0 * mpl * pow(tau - tp, 2) * (tm - tp) * (3 * pow(tau, 2) + 2 * tau * tp + pow(tp, 2)) /
                   pow(mpl * (tau - tp) - mpr * (tau - tm), 2);
        default:
            printError("AGM::Greenfunction::integrate_square_tau", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
    ...
    switch (i) {
        ...
        case 'r':
            return -1.0 / 6.0 * mpl * pow(tau - tp, 2) * (2 * tau + tp) * (tm - tp) /
                   pow(mpl * (tau - tp) - mpr * (tau - tm), 2);
        default:
            printError("AGM::Greenfunction::integrate_linear_tau", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_const_tau(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_const_tau", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
    ...
    switch (i) {
        ...
        case 'r':
            return -1.0 / 3.0 * mpl * (tau - tp) * (tm - tp) * (pow(tau, 2) + tau * tp + pow(tp, 2)) /
                   pow(mpl * (tau - tp) - mpr * (tau - tm), 2);
        default:
            printError("AGM::Greenfunction::integrate_square_ttau", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_linear_ttau(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_linear_ttau", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_const_ttau(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_const_ttau", "char i (which is %c) wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_square_ND(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_square_ND", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_linear_ND(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_linear_ND", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_const_ND(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_const_ND", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_square_t_ND(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_square_t_ND", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_linear_t_ND(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_linear_t_ND", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_const_t_ND(char i) const -> double {
    switch (i) {
        ...
        case 'r':
            return (tau - tp) / mpr;
        default:
            printError("AGM::Greenfunction::integrate_const_t_ND", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_square_tau_ND(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_square_tau_ND", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_linear_tau_ND(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_linear_tau_ND", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_const_tau_ND(char i) const -> double {
    switch (i) {
        ...
        case 'l':
            return (tm - tp) / mpr;
        default:
            printError("AGM::Greenfunction::integrate_const_tau_ND", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}

auto AGM::Greenfunction::integrate_square_ttau_ND(char i) const -> double {
    return ZEROVALUE;
}

auto AGM::Greenfunction::integrate_linear_ttau_ND(char i) const -> double {
    return ZEROVALUE;
}

auto AGM::Greenfunction::integrate_const_ttau_ND(char i) const -> double {
    return ZEROVALUE;
}
...
auto AGM::Greenfunction::integrate_square_DN(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_square_DN", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_linear_DN(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_linear_DN", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_const_DN(char i) const -> double {
    switch (i) {
        ...
        default:
            printError("AGM::Greenfunction::integrate_const_DN", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_square_t_DN(char i) const -> double {
    switch (i) {
        ...
        case 'r':
            return ZEROVALUE;
        default:
            printError("AGM::Greenfunction::integrate_square_t_DN", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_linear_t_DN(char i) const -> double {
    switch (i) {
        ...
        case 'r':
            return ZEROVALUE;
        default:
            printError("AGM::Greenfunction::integrate_linear_t_DN", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_const_t_DN(char i) const -> double {
    switch (i) {
        ...
        case 'r':
            return ZEROVALUE;
        default:
            printError("AGM::Greenfunction::integrate_const_t_DN", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_square_tau_DN(char i) const -> double {
    switch (i) {
        ...
        case 'r':
            return (1.0 / 3.0) * (-pow(tm, 3) + pow(tp, 3)) / mpr;
        default:
            printError("AGM::Greenfunction::integrate_square_tau_DN", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_linear_tau_DN(char i) const -> double {
    switch (i) {
        ...
        case 'r':
            return (1.0 / 2.0) * (-pow(tm, 2) + pow(tp, 2)) / mpr;
        default:
            printError("AGM::Greenfunction::integrate_linear_tau_DN", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}
...
auto AGM::Greenfunction::integrate_const_tau_DN(char i) const -> double {
    switch (i) {
        ...
        case 'r':
            return (-tm + tp) / mpr;
        default:
            printError("AGM::Greenfunction::integrate_const_tau_DN", "char i (which is %c) is wrong", i);
            return ZEROVALUE;
    }
}

auto AGM::Greenfunction::integrate_square_ttau_DN(char i) const -> double {
    return ZEROVALUE;
}

auto AGM::Greenfunction::integrate_linear_ttau_DN(char i) const -> double {
    return ZEROVALUE;
}

auto AGM::Greenfunction::integrate_const_ttau_DN(char i) const -> double {
    return ZEROVALUE;
}
