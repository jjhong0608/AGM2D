...
auto AGM::point::getAlinMaxGap() -> double {
  return alin_max_gap;
}
...
void AGM::point::findStencilInterface() {
  ...
  auto assignAppendElement = [this](point &pt, EWNS ewns, const coordinate &coordinate) -> void {
    auto func = [this, &pt, &coordinate](EWNS ewns0, EWNS ewns1, EWNS ewns2, EWNS ewns3, int i) -> void {
      ...
      else if (isnegative(coordinate[i]))
        pt[ewns3] = &pt;
      else
        pt[ewns2] = pt[ewns3] = &pt;
    };
    if (ewns == E) func(ewns, W, N, S, 1);
      ...
      printError("AGM::point::findStencilInterface", "assignAppendElement");
  };
  auto assignStencil = [&](EWNS ewns, EWNS ewns0, EWNS ewns1, auto func, char lineChar, int lineIdx,
    ...
    if (alin) {
      ...
      if (crossLine.size() > 1) {
        ...
        printInformation();
        printError("AGM::point::findStencilInterface", "The size of crossLine is empty");
      }
...
void AGM::point::calculateRepresentationFormulaDirichlet(int order) {
  auto findInnerPointOfBoundary = [this](int i) -> point * {
    ...
    printInformation();
    printError("AGM::point::calculateRepresentationFormulaDirichlet", "findInnerPointOfBoundary");
    return {};
  };
...
void AGM::point::calculateRepresentationFormulaNeumann(int order) {
  ...
  auto findInnerPointOfBoundary = [this](int i) -> point * {
    ...
    printInformation();
    printError("AGM::point::calculateRepresentationFormulaNeumann", "findInnerPointOfBoundary");
    return {};
  };
  auto pt1{*findInnerPointOfBoundary(1)};
  auto pt2 = order > 0 ? *findInnerPointOfBoundary(2) : point();
...
void AGM::point::calculateRepresentationFormulaInterface() {
  ...
  auto assignMatrix = [&](point *pt, point *ptr, point *ptl, double mp0, Greenfunction *func, double d, int i, int i0, char c, char C) -> void {
    ...
    if (pt) {
      ...
      row[i0][pt->getIdx() + getNPts()] += isInterface ? sign * func->green_integral(C, ord) : sign * func->green_integral(c, ord);
      ...
      row[i0] += isInterface ? linearApproximation(ptr, ptl, sign * func->green_integral(C, ord), i, 1) : linearApproximation(ptr, ptl, sign * func->green_integral(c, ord), i, 1);
...
auto AGM::point::calculateRepresentationFormulaInterfaceNeumannOnAxial(
    char axis,
    int axisInt,
    AGM::EWNS ewns) -> AGM::matrixRow {
  std::string string = ewns == E || ewns == N ? "ND" : ewns == W || ewns == S ? "DN"
                                                                              : "";
...
void AGM::point::approximatePhiAtBoundary1(int order) {
  ...
  auto thirdOrderExtrapolation = [this, &axisInt, &pt1, &pt2, &pt3, &pt4]() -> void {
    ...
    auto thirdOrder = [](double t, double t1, double t2, double t3, double t4) -> double {
      return (std::pow(t, 3) - std::pow(t, 2) * (t2 + t3 + t4) + t * (t2 * t3 + t2 * t4 + t3 * t4) - t2 * t3 * t4) / (std::pow(t1, 3) - std::pow(t1, 2) * (t2 + t3 + t4) + t1 * (t2 * t3 + t2 * t4 + t3 * t4) - t2 * t3 * t4);
    };
  ...
  auto fourthOrderExtrapolation = [this, &axisInt, &pt1, &pt2, &pt3, &pt4, &pt5]() -> void {
    ...
    auto fourthOrder = [](double t, double t1, double t2, double t3, double t4, double t5) -> double {
      return (std::pow(t, 4) - std::pow(t, 3) * (t2 + t3 + t4 + t5) + std::pow(t, 2) * (t2 * t3 + t2 * t4 + t2 * t5 + t3 * t4 + t3 * t5 + t4 * t5) - t * (t3 * t4 * t5 + t2 * t4 * t5 + t2 * t3 * t5 + t2 * t3 * t4) + t2 * t3 * t4 * t5) / (std::pow(t1, 4) - std::pow(t1, 3) * (t2 + t3 + t4 + t5) + std::pow(t1, 2) * (t2 * t3 + t2 * t4 + t2 * t5 + t3 * t4 + t3 * t5 + t4 * t5) - t1 * (t3 * t4 * t5 + t2 * t4 * t5 + t2 * t3 * t5 + t2 * t3 * t4) + t2 * t3 * t4 * t5);
    };
...
void AGM::point::updateRightHandSideInterface(const std::function<double(int)> &f, const std::function<double(int)> &g) {
...
void AGM::point::updateRightHandSideDirichletPart(const std::function<double(int)> &f, const std::function<double(int)> &g) {
}

void AGM::point::updateRightHandSideNeumannPart(const std::function<double(int)> &f, const std::function<double(int)> &g) {
...
void AGM::point::updateRightHandSideInterfacePart(const std::function<double(int)> &f, const std::function<double(int)> &g) {
  bool isinterface{solMatrixRow[1].size() == 1};
  for (const auto &item : partMatrixRow[0]) {
    if (item.idx < getNPts()) {
      rb[0] -= item.value * f(item.idx);
...
void AGM::point::updateRightHandSidePhiPressure(
...
void AGM::point::approximateNaNDerivatives(std::vector<AGM::point> *points) {
  ...
  if (std::isnan(values["dxx"])) values["dxx"] = findInnerPointOfBoundary()->getValue()["dxx"];
  if (std::isnan(values["dy"])) values["dy"] = findInnerPointOfBoundary()->getValue()["dy"];
  if (std::isnan(values["dyy"])) values["dyy"] = findInnerPointOfBoundary()->getValue()["dyy"];
}
